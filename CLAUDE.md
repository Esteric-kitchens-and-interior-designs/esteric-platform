...
- `apps/web` — public marketing site. Home, About, service pages (Kitchen Designs, Interior Designs, Landscaping, Wardrobes & Cabinets), Portfolio, Blog, Careers, Contact, Quote Request, Appointment Booking, Newsletter, legal pages. Testimonials, Certifications & Awards, and FAQs are homepage sections (`/#testimonials`, `/#certifications`, `/#faqs`), not standalone routes. Public forms write directly into CRM tables (`Lead`, `AppointmentBooking`, `NewsletterSubscriber`, `JobApplication`) — there is no separate "submissions" inbox, the CRM leads/careers modules *are* the inbox.
...
- **No payments package.** This business never takes payment through the website or CRM (deposits/invoices are handled offline) — `packages/payments` (Stripe, from the next-forge SaaS template) was removed entirely rather than left dormant. Don't reintroduce it without being asked.
